...
Deno.test("little endian u8 length array of f64", async () => {
  const testArrays = [
    [],
    [0.5342],
    [634562675.2457345, 4294967295.62345, 7346526.52346],
  ...
  for (let i = 0; i < testArrays.length; i++) {
    const dataArr = testArrays[i];
    const data = await u8LenArr(f64(), Endian.Little).encode(dataArr);
    assertEquals(data.length, 1 + 8 * dataArr.length);
    const cursor = new Cursor(0);
    assertEquals(
...
Deno.test("little endian u16 length array of f64", async () => {
  ...
  for (let i = 0; i < testArrays.length; i++) {
    const dataArr = testArrays[i];
    const data = await u16LenArr(f64(), Endian.Little).encode(dataArr);
    assertEquals(data.length, 2 + 8 * dataArr.length);
    const cursor = new Cursor(0);
    assertEquals(
...
Deno.test("little endian u32 length array of f64", async () => {
  ...
  for (let i = 0; i < testArrays.length; i++) {
    const dataArr = testArrays[i];
    const data = await u32LenArr(f64(), Endian.Little).encode(dataArr);
    assertEquals(data.length, 4 + 8 * dataArr.length);
    const cursor = new Cursor(0);
    assertEquals(
